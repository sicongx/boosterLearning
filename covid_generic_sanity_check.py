...
def load_covid_long_format(rnn_data_path: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    raw = raw.copy()
    raw["Age.FirstDose"] = raw["Age.FirstDose"].replace(">89", "90")
    raw["Age.FirstDose"] = pd.to_numeric(raw["Age.FirstDose"], errors="raise").astype(int)
...
# -----------------------------------------------------------------------------
# Functions for the COVID example
# -----------------------------------------------------------------------------
...
def build_rnn_training_dataset(long_df: pd.DataFrame, seed: int) -> TrajectoryDataset:
    """Helper for refitting the RNN with the original masking logic.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--save-results", default=None, help="Optional path to save summary results (.npz)")
...
if __name__ == "__main__":
    ...
    if args.save_results is not None:
        np.savez(
            ...
            train_repeats=args.train_repeats,
            eval_epochs=args.eval_epochs,
            table=out.reward_summary["table"],
            data=out.reward_summary["data"],
            all_policy=out.reward_summary["all"],
            none=out.reward_summary["none"],
            epoch_reward_shape=np.array(out.epoch_reward_shape),
            real_inf_rate_first5=out.real_inf_rate_first5,
            real_sev_rate_first5=out.real_sev_rate_first5,
        )
